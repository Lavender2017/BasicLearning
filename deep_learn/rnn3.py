...
mnist = input_data.read_data_sets('D:\python_communication\workspace\TensorFlow\data\Mnist_data', one_hot=True)
...
def recurrent_neural_network(data):
    ...
    data = tf.transpose(data, [1, 0, 2])#转置矩阵
    data = tf.reshape(data, [-1, chunk_size])#28*60000,28

    data = tf.split(0, chunk_n, data)#按行分成28*60000个数组
